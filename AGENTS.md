...
- Design: "Seattle 2016" flat design — college navy + action green, Montserrat uppercase display + Open Sans body, Material-style `.card` surfaces, Space Needle SVG motifs. Tokens live in `app/globals.css` — use those, not ad-hoc colors. Section cards/panels use `RetroPanel` (name is a holdover; it renders a flat card).
- Certificates (`components/Certificate.tsx`) stay print-clean and restrained — don't apply the loud retro elements there.
